...
glmdata$CompanyAge <- cut(glmdata$CompanyAge,
                          breaks = c(-Inf, 1, 5, 10, 15, Inf),
                          labels = c("01_<1", "02_1_5", "03_5-10", "04_10-15", "05_>=15"),
                          right = FALSE)
...
glmdata3 <-
  data.frame(rating.factor =
               c(rep("NoPGroup", nlevels(glmdata2$NoP_group)),
                 ...
                 rep("CompanyAge", nlevels(glmdata2$CompanyAge))),
             class =
               c(levels(glmdata2$NoP_group),
                 levels(glmdata2$Activity_group),
                 levels(glmdata2$CompanyAge)),
             stringsAsFactors = FALSE)

new.cols <-
  foreach (rating.factor = c("NoP_group", "Activity_group", "CompanyAge"),
           .combine = rbind) %do%
  {
...
model.frequency <-
  glm(NumberOfClaims ~ NoP_group + Activity_group + CompanyAge+ offset(log(Duration)),
...
model.severity <-
  glm(avgclaim ~ NoP_group + Activity_group + CompanyAge ,
...
p7 <- ggplot(subset(glmdata3, rating.factor=="CompanyAge"), aes(x=class, y=rels.frequency)) +
  geom_point(colour="blue") + geom_line(aes(group=1), colour="blue") + ggtitle("ComapnyAge: frequency factors") +
  geom_text(aes(label=paste(round(rels.frequency,2))), nudge_y=1) +theme(axis.text.x = element_text(angle = 30, hjust = 1))

p8 <- ggplot(subset(glmdata3, rating.factor=="CompanyAge"), aes(x=class, y=rels.severity)) +
  geom_point(colour="blue") + geom_line(aes(group=1), colour="blue") + ggtitle("CompanyAge: severity factors") +
  geom_text(aes(label=paste(round(rels.severity,2))), nudge_y=0.5)+theme(axis.text.x = element_text(angle = 30, hjust = 1))

p9 <- ggplot(subset(glmdata3, rating.factor=="CompanyAge"), aes(x=class, y=rels.risk)) +
  geom_point(colour="blue") + geom_line(aes(group=1), colour="blue") + ggtitle("CompanyAge: risk factors") +
  geom_text(aes(label=paste(round(rels.risk,2))), nudge_y=1.6)+theme(axis.text.x = element_text(angle = 30, hjust = 1))
...
#fit full model
model_full <- glm(NumberOfClaims ~ NoP_group + Activity_group + CompanyAge + offset(log(Duration)),
                  data = glmdata2, family = poisson)

#fit reduced model
model_reduced <- glm(NumberOfClaims ~ NoP_group  + CompanyAge+  offset(log(Duration)),
                     data = glmdata2, family = poisson)
